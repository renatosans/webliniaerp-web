app.controller('RelatorioDREController', function($scope, $http, $window, UserService,EmpreendimentoService) {
	var ng 				= $scope,
		aj 				= $http;
		...
		aj.get(url)
			.success(function(data, status, headers, config) {
				angular.forEach(data, function(item, index){
					if (item.flg_total_faturamento == 1){
						ng.prc_total_faturamento = item.valor_calculado;
					}
				});
				angular.forEach(data, function(item, index){
					item.prc_total_faturamento_item = item.valor_calculado / ng.prc_total_faturamento;
				});
