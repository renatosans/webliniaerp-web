app.controller('PDVController', function($scope, $http, $window,$dialogs, UserService,ConfigService,CaixaService,$timeout,FuncionalidadeService,PrestaShop,TabelaPrecoService,$sce) {
		...
		aj.get(baseUrlApi() + 'venda/orcamento/' + id_orcamento + '/' + tipo_valor)
			.success(function(data, status, headers, config) {
				...
				if(Number(data.cliente.id) != Number(ng.configuracoes.id_cliente_movimentacao_caixa)) {
					aj.get(baseUrlApi()+"usuarios/saldodevedor/"+ ng.userLogged.id_empreendimento+"?usu->id="+data.cliente.id)
						.success(function(data, status, headers, config) {
	...
	ng.changeFlagImprimirNFCe = function() {
		var actual_value = angular.copy(ng.caixa_aberto.flg_imprimir_nfce);
		if(parseInt(actual_value, 10) === 1)
			ng.caixa_aberto.flg_imprimir_nfce = 0;
		else
			...
			aj.post(baseUrlApi()+"venda/gravarMovimentacoes",{ 
				...
				if(!ng.pagamento_fulso && Number(ng.caixa_aberto.flg_imprimir_nfce) == 1){
					...
					if(empty(ng.caixa_open.id_ws_dsk)){
						...
						$('#modal_progresso_venda').modal('hide');
						ng.showModalSatCfe('Aguarde, emitindo CF-e SAT...');
						var post = { 
							id_empreendimento : ng.userLogged.id_empreendimento,
					...
					if(
						(!ng.pagamento_fulso) && 
						(!empty(ng.configuracoes.flg_imprimir_cnf_antes_de_fechar_guia) && ng.configuracoes.flg_imprimir_cnf_antes_de_fechar_guia == 1)
					) {
						ng.printTermic(true);
					}
					else { 
	...
	ng.loadCliente= function(offset,limit) {
		...
		ng.clientes = null;
		query_string = "?(tue->id_empreendimento[exp]=="+ng.userLogged.id_empreendimento+"&usu->flg_excluido=0"+"&usu->id[exp]= NOT IN("+ng.configuracoes.id_cliente_movimentacao_caixa+","+ng.configuracoes.id_usuario_venda_vitrine+"))";

		if(ng.busca.clientes != ""){
			query_string += "&"+$.param({'(usu->nome':{exp:"like'%"+ng.busca.clientes+"%' OR tpf.cpf LIKE '%"+ng.busca.clientes+"%' OR tpj.cnpj LIKE '%"+ng.busca.clientes+"%' OR tpj.nome_fantasia LIKE '%"+ng.busca.clientes+"%' OR usu.apelido LIKE '%"+ng.busca.clientes+"%')"}});
		}
	...
	ng.aplicarRecebimento = function(){
		...
		if(push){
			if(ng.pagamento.id_forma_pagamento == 8){ // Transferência Bancária
				var item = {
	...
	ng.selectChange = function(forma_pagamento){
		...
		switch(ng.pagamento.id_forma_pagamento) {
			case 2: // Cheque
		...
		ng.loadBandeiras(forma_pagamento.id);
		ng.loadDatapicker();
	}
