app.controller('DevolucaoController', function($scope, $http, $window,$dialogs, UserService){
	...
	ng.loadClientes = function(offset, limit) {
		...
		if(ng.busca.clientes != ""){
			query_string += "&"+$.param({"(usu->nome":{exp:"like'%"+ng.busca.clientes+"%' OR tpf.cpf LIKE '%"+ng.busca.clientes+"%' OR apelido like '%"+ng.busca.clientes+"%')"}})+"";
		}
	...
	ng.loadCliente= function(offset,limit) {
		...
		if(ng.busca.clientes != ""){
			query_string += "&"+$.param({'(usu->nome':{exp:"like'%"+ng.busca.clientes+"%' OR tpf.cpf LIKE '%"+ng.busca.clientes+"%' OR usu.apelido LIKE '%"+ng.busca.clientes+"%')"}});
		}
