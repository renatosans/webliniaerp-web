app.controller('DashboardController', function($scope, $http, $window, UserService,ConfigService,FuncionalidadeService,TabelaPrecoService) {
	try {
		...
		ng.aplicarFiltro = function() {
			...
			ng.loadVendasHoraHoraSemanal(date_first, date_last);
		}

		ng.changeGraficoHoraHora = function(){
			var date_first = formatDate($("#dtaInicial").val());
			var date_last  = formatDate($("#dtaFinal").val());
			ng.loadVendasHoraHoraSemanal(date_first, date_last);	
		}
		...
		ng.redirectToSubReport = function(report_name,format_date,filter) {
			...
			window.location.href = report_name + '.php?dtaInicial='+ dta_inicial +'&dtaFinal='+ dta_final +'&filter='+ filter;
		}
		...
		ng.vendas_hora_hora = null ;
		ng.loadVendasHoraHoraSemanal = function(date_first, date_last){
			var queryString = "?id_empreendimento="+ng.userLogged.id_empreendimento;
			var url = baseUrlApi()+"relatorio/venda_hora_a_hora/"+ng.userLogged.id_empreendimento+"/"+date_first+"/"+date_last;
			...
			aj.get(url)
				.success(function(data, status, headers, config) {
					...
					var moeda = "";
					var em = "";
					if (ng.flg_tipo_map_hora_hora == 1) {
						angular.forEach(data.semana, function(horas, diaSemana){
							var idxHora = 0;
								angular.forEach(horas, function(dados, hora){
									idxHora++;
									items_arr.push([idxDiaSemana, idxHora, dados.qtd])
								});
							idxDiaSemana++;
						});
					} else {
						angular.forEach(data.semana, function(horas, diaSemana){
							var idxHora = 0;
								angular.forEach(horas, function(dados, hora){
									idxHora++;
									items_arr.push([idxDiaSemana, idxHora, dados.total])
								});
							idxDiaSemana++;
						});
						moeda = "R$ ";
					...
					Highcharts.chart('container_teste', {
						chart: {
							type: 'heatmap',
							marginTop: 40,
							marginBottom: 80,
						...
						yAxis: {
							categories: [ 	'0', '00:00 - 01:00','01:00 - 02:00','02:00 - 03:00','03:00 - 04:00','04:00 - 05:00','05:00 - 06:00','06:00 - 07:00','07:00 - 08:00','08:00 - 09:00','09:00 - 10:00','10:00 - 11:00','11:00 - 12:00',
						...
						tooltip: {
							formatter: function () {
								...
								moeda + this.point.value + '</b>' + em + ' Vendas <br><b>' + this.series.yAxis.categories[this.point.y] + '</b>';
							}
						},
