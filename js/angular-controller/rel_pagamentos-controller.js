app.controller('relPagamentosController', function($scope, $http, $window, $dialogs, UserService, ConfigService, FuncionalidadeService, EmpreendimentoService){
    ...
    ng.formas_pagamento = [
    	{ value: '', dsc:'Todas' },
    	{ value: 4, dsc:'Boleto Bancário' },
    	{ value: 6, dsc:'Cartão de Crédito' },
    	{ value: 5, dsc:'Cartão de Débito' },
    	{ value: 2, dsc:'Cheque' },
    	{ value: 3, dsc:'Dinheiro' },
    	{ value: 8, dsc:'Transferência Bancária' },
    	{ value: 7, dsc:'Vale-Troca' },
    	{ value: 10, dsc:'Voucher' }
    ];

    ng.tipos_status = [
    	{ value: '', dsc:'Todos' },
    	{ value: 0, dsc:'Pendente' },
    	{ value: 1, dsc:'Pago' }
    ];
	...
	ng.loadMovimentacoes= function() {
		...
		if (ng.busca.id_forma_pagamento.length == 1){
			query_string += !empty(ng.busca.id_forma_pagamento) ? "&tpv->id_forma_pagamento="+ng.busca.id_forma_pagamento[0] : "" ;
		} else {
			query_string += "&tpv->id_forma_pagamento[exp]= IN("+ng.busca.id_forma_pagamento+")";
		}
	...
	if (params.dtaInicial && params.dtaInicial) {
		if (params.filter == 'pc') {
			ng.busca_aux.status_pagamento = 1;
		} else if (params.filter == 'cc') {
			ng.busca_aux.id_forma_pagamento = 6;
			ng.busca_aux.status_pagamento = 0;
		} else if(params.filter == 'bc'){
			ng.busca_aux.id_forma_pagamento = 4;
			ng.busca_aux.status_pagamento = 0;
		} else if(params.filter == 'tm'){
			ng.busca_aux.id_forma_pagamento = [5,6];
		}
		ng.loadMovimentacoes();
	}
});
