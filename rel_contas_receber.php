...
<html lang="en" ng-app="HageERP">
  ...
  <body class="overflow-hidden" ng-controller="RelatorioContasReceber" ng-cloak>
	<!-- Overlay Div -->
				...
				<div class="clearfix">
					<div class="pull-left">
						...
						<div class="pull-left m-left-sm">
							...
							<span class="text-muted">Saldo de Pagamentos por Cliente</span>
						</div>
					</div>
							...
							<div class="row">
								...
								<div class="col-lg-6">
									<div class="form-group">
										...
										<div class="input-group">
											<input ng-click="selCliente(0,10)" type="text" class="form-control" ng-model="cliente.nome" readonly="readonly" style="cursor: pointer;"></input>
											<span class="input-group-btn">
												<button ng-click="selCliente(0,10)" type="button" class="btn"><i class="fa fa-truck"></i></button>
											</span>
										</div>
									</div>
								</div>
				...
				<table id="data" class="table table-bordered table-hover table-striped table-condensed">
					<thead>
						<tr>
							...
							<th class="text-center" colspan="3">Taxa Maquineta</th>
						</tr>
					</thead>
					<tbody>
						<tr ng-repeat="item in pagamentos">
							<td class="text-center">{{ item.data_pagamento | dateFormat: 'date' }}</td>
							<td>{{ item.nome }}</td>
							<td class="text-center">{{ item.cod_plano}} - {{ item.dsc_plano }}</td>
							<td class="text-center">{{ item.dsc_conta_bancaria }}</td>
							<td class="text-center">{{ item.descricao_forma_pagamento }}</td>
							<td class="text-right">R$ {{ item.valor_pagamento | numberFormat:2:',':'.' }}</td>
							<td class="text-right" width="70">{{ item.taxa_maquineta * 100 | numberFormat:2:',':'.' }} %</td>
							<td class="text-right" width="70">R$ {{ item.valor_pagamento * item.taxa_maquineta | numberFormat:2:',':'.' }}</td>
							<td class="text-right" width="100">R$ {{ item.valor_pagamento - (item.valor_pagamento * item.taxa_maquineta) | numberFormat:2:',':'.' }}</td>
						</tr>
					</tbody>
				</table>
				<div class="pull-right">
	...
	<script src="js/angular-controller/relatorio-contas-receber.js"></script>
